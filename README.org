...
#+html:<details>
#+html:<summary><strong>inbox</strong> - Per-session socket for async events into the conversation</summary>
#+html:<br>

- *Source*: [[https://github.com/rytswd/pi-agent-extensions/tree/main/inbox][inbox/]]
- *License*: MIT
- *Env*: =$PI_INBOX= (socket path), =$PI_SESSION_ID=

*Description*: Tool calls are synchronous, so once a background job is detached the agent normally has to poll. This extension listens on =$XDG_RUNTIME_DIR/pi-inbox/<pid>.sock= and exports the path as =$PI_INBOX= to every tool subprocess. Each connection becomes one message (JSON ={"source": "...", "pid": 123, "text": "..."}= or plain text, 16 KB cap) that is delivered right away when the agent is idle, otherwise as a follow-up after the current run, and rendered as a compact =▌inbox <source> pid <n>= block. When =pid= is omitted the sender's PID is read from the socket peer credentials (=SO_PEERCRED= / =LOCAL_PEERPID= via =python3=), so the agent can tell which of its background jobs is reporting.

#+begin_src bash
echo '{"source":"ci","text":"build green"}' | nc -U "$PI_INBOX"
#+end_src

Producers: [[https://github.com/Mic92/mics-skills/tree/main/queue][queue]] reports detached tasks finishing, the =python= extension exposes =notify()=.

#+html:</details>

#+html:<details>
#+html:<summary><strong>python</strong> - Persistent Python interpreter tool</summary>
#+html:<br>

- *Source*: [[https://github.com/rytswd/pi-agent-extensions/tree/main/python][python/]]
- *License*: MIT
- *Requirements*: =python3=
- *Config*: =~/.config/pi-agent-extensions/python/config.json= with optional =python= (interpreter path) and =prompt= (appended to the tool description: installed packages, house rules)
- *Command*: =/python-restart=

*Description*: Registers a =python= tool backed by one long-lived interpreter per session (=driver.py=, JSON lines over private fds so user code cannot corrupt the protocol). Variables, imports and =pexpect= children survive between calls, a trailing expression is echoed like in a REPL, matplotlib figures come back as images, output is tail-truncated and a =timeout= interrupts the cell without losing state. With the =inbox= extension loaded a =notify(text, source="python")= helper is predefined so background threads can report back asynchronously.

Example =config.json= via home-manager, giving the tool its own interpreter with a few libraries and telling the model about them:

#+begin_src nix
{ pkgs, ... }:
let
  python = pkgs.python3.withPackages (ps: [ ps.polars ps.matplotlib ps.requests ps.pexpect ]);
in
{
  xdg.configFile."pi-agent-extensions/python/config.json".text = builtins.toJSON {
    python = "${python}/bin/python3";
    prompt = "Available besides stdlib: polars, matplotlib, requests, pexpect. Drive interactive programs with pexpect, always pass timeout= to expect().";
  };
}
#+end_src
...
- *python:* Ask it to analyse a CSV. State persists between calls
- *inbox:* =echo hi | nc -U $PI_INBOX= from a tool subprocess wakes the agent
